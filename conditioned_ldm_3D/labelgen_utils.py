...
def log_mlflow(
    ...
    if local:
        mlflow.set_tracking_uri("file:/home/vf19/PycharmProjects/brainSPADE3D/mlruns")
    else:
        mlflow.set_tracking_uri("file:/code/mlruns")
...
def getLearningRate(optimizer, warmup_lr, base_lr, n_epochs_warmup,
    ...
    if lr_scheduler_type == 'constant':
        lambda1 = lambda epoch: int(0 <= epoch < n_epochs_warmup) * (1.0) \
                                ...
                                + int(n_epochs_shift <= epoch) * (base_lr/warmup_lr) * 1.0
    ...
    for e in range(n_epochs):
        ...
        lrs.append(get_lr(dummy_optimizer))
        if np.isnan(lrs[-1]):
