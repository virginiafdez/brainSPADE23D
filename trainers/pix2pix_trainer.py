...
class Pix2PixTrainer():
    ...
    def run_generator_one_step(self, data, with_gradients = False, epoch = None):
        ...
        if self.self_supervised:
            ...
            g_losses_nw['self_supervised'] = ss_loss.item()
        if self.opt.second_style_slice:
            sec_style_loss = self.second_style_slice_loss(data, z)
            g_losses['second_style'] = sec_style_loss * self.ss_w
            g_losses_nw['second_style'] = sec_style_loss.item()
    ...
    def run_encoder_one_step(self, data, dataset = None):
        ...
        if self.opt.second_style_slice:
            sec_style_loss = self.second_style_slice_loss(data, z)
            g_losses['second_style'] = sec_style_loss * self.ss_w
            g_losses_nw['second_style'] = sec_style_loss
    ...
    def second_style_slice_loss(self, data, codes):

        '''
        Forward passes second styles, gets codes and calculates distance between original style encoding vector
        and this one.
        :param data:
        :param codes:
        :return: distance metric (l2, l1 or cosine similarity, as per self.opt.distance_metric)
        '''

        data_aug_dict = data.copy()
        data_aug_dict['style_image'] = data['second_style']
        codes_ss, _ = self.pix2pix_model_on_one_gpu(data_aug_dict, 'encode_only')
        second_style_loss = self.distance(codes, codes_ss).mean()
        return second_style_loss
    ...
    def run_tester_one_step(self, data, get_code = False, epoch = None):
        ...
        if self.self_supervised:
            ...
            g_losses_nw['self_supervised'] = ss_loss
        if self.opt.second_style_slice:
            sec_style_loss = self.second_style_slice_loss(data, z).item()
            g_losses['second_style'] = sec_style_loss * self.ss_w
            g_losses_nw['second_style'] = sec_style_loss
