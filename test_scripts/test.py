...
test_flags = [False, False, False, False, False, False, False, True, False, False, False]  # Which test sections run
...
if test_flags[1]:
    ...
    if mod_disc is not None:
        with open(os.path.join(all_path, 'accuracy.txt'), 'w') as f:
            f.write("Modality\tSlice\tAccuracy Mod (%)\tAccuracy Dat (%)\n")
...
if test_flags[4] and opt.dataset_type == 'volume':
    ...
    modalities = opt.sequences
    number_of_volumes = 10  # Number of volumes per modality
