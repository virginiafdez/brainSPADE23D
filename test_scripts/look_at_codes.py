...
if do_std:
    ...
    if not os.path.exists(os.path.join(opt.savecode_dir, 'std_plots')):
        os.makedirs(os.path.join(opt.savecode_dir, 'std_plots'))
    ...
    for mod in mods:
        ...
        for i, data_i in enumerate(dataloader):
            variances = []
            seqs = data_i['this_seq']
            for forward_time in range(FORWARD):
                ...
                variances.append(generated.numpy())
            variances = np.stack(variances, 1) # B x FORWARDED TIMES x CODE
            variances = np.std(variances, 1)
            for item_b in range(variances.shape[0]):
                if seqs[item_b] not in codes.keys():
                    codes[seqs[item_b]] = []
                codes[seqs[item_b]].append(variances[item_b, ...]) # Mean across codes.

    torch.save(codes, os.path.join(opt.savecode_dir, 'std_plots', 'result.pth'))
    # Plot
    codes_mean_code = {}
    codes_mean_inst = {}
    for mod, code_values in codes.items():
        cat_codes = np.stack(code_values, 0)
        codes_mean_code[mod] = np.mean(cat_codes, 0) # >>> 1 X CODE ELEMENTS
        codes_mean_inst[mod] = np.mean(cat_codes, 1) # >>> N_SUBJECT X 1

    for mod in list(codes.keys()):
        plt.subplot(2,1,1);
        plt.boxplot(codes_mean_inst[mod]);
        plt.title("Distribution of mean variance of the code across subject", fontsize = 10)
        plt.subplot(2,1,2);
        plt.bar(range(len(codes_mean_code[mod])), codes_mean_code[mod])
        plt.xlabel("Code index")
        plt.ylabel("Variance across all subjects")
        plt.suptitle("%s" %mod)
        plt.savefig(os.path.join(opt.savecode_dir, 'std_plots', '%s_result.png' %mod))
        plt.close('all')
...
if do_volume and opt.dataset_type == 'volume':
    ...
    number_of_volumes = 30  # Number of volumes per modality
