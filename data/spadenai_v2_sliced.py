...
class SpadeNaiSlice():
    ...
    def __init__(self, opt, mode, **kwargs):
        ...
        if self.non_corresponding_dirs:
            self.non_corresponding_slices = False # Images = Styles DIFFERENT FROM LABELS.
        ...
        if self.fix_seq:
            if self.non_corresponding_dirs and not opt.isTrain:
                if self.style_label_dir is None and self.skullstrip:
                    raise ValueError("Flag non_corresponding_dirs is True, label_dir and image_dir are not corresponding."
                               "You need to specify style_label_dir if you are to skullstrip.")
    ...
    def getIteratorFun(self, dataset):
        ...
        for volumes in dataset:
            ...
            if 'second_slice' in volumes.keys():
                if isinstance(volumes['second_slice'], monai.data.meta_tensor.MetaTensor):
                    volumes['second_slice'] = np.asarray(volumes['second_slice'])
            if 'second_slice_label' in volumes.keys():
                if isinstance(volumes['second_slice_label'], monai.data.meta_tensor.MetaTensor):
                    volumes['second_slice_label'] = np.asarray(volumes['second_slice_label'])
            ...
            if self.second_style_slice:
                second_slice = torch.from_numpy(volumes['second_slice'])
                second_slice_label = torch.from_numpy(volumes['second_slice_label'])[..., 0, :]

            # Pad to fit the self.size
            label = self.padder(label, islabel = True)
            image = self.padder(image, islabel = False)
            if self.second_style_slice:
                second_slice = self.padder(second_slice, islabel = False)
                second_slice_label = self.padder(second_slice_label, islabel = True)
            ...
            if self.non_corresponding_slices:
                ...
                if self.skullstrip:
                    label_style = torch.from_numpy(volumes['style_label_file'])
                    if len(label_style.shape) > 3:
                        label_style = label_style[..., 0, :]
                else:
                    label_style = np.zeros_like(label)
            ...
            if self.second_style_slice:
                second_slice = second_slice.permute(2, 0, 1)
            ...
            if self.second_style_slice:
                second_slice = self.normalize(second_slice)
            ...
            if self.skullstrip:
                ...
                if self.second_style_slice:
                    second_slice = uvir.SkullStrip(second_slice, second_slice_label, value=-1)
            ...
            if self.second_style_slice:
                out_dict['second_style'] = second_slice
            yield out_dict
        ...
        while length_all > 0:
            ...
            if self.diff_style_volume:
                ...
                if len(possible_ims) == 0:
                    ...
                    if self.second_style_slice:
                        style_name = np.random.choice([i for i in all_images[chosen_mod] if i != image_name], 2)
                        second_slice = style_name[1]
                        style_name = style_name[0]
                    else:
                        style_name = np.random.choice([i for i in all_images[chosen_mod] if i != image_name])
                else:
                    if self.second_style_slice:
                        style_name = np.random.choice([i for i in all_images[chosen_mod] if dataset not in i], 2)
                        second_slice = style_name[1]
                        style_name[0]
                    else:
                ...
                if self.second_style_slice:
                    second_slice_label = second_slice.replace(chosen_mod, "Parcellation", 1)
            elif self.diff_slices:
                # We look for slices from the same volume, but different slices.
                root = "_".join(image_name.split("_")[:-2]) # -1 is slice number, -2 if lesion + slice number
                ...
                if len(possible_ims) == 0:
                    ...
                    if self.second_style_slice:
                        second_slice = image_name
                        second_slice_label = label_name
                else:
                    if self.second_style_slice:
                        style_name = np.random.choice(possible_ims, 2)
                        second_slice = style_name[1]
                        style_name = style_name[0]
                    else:
            ...
            if self.diff_style_volume or self.diff_slices:
                out_item = {'image_file': os.path.join(self.image_dir, image_name),
                            ...
                            'slice_no': int(image_name.strip(".%s" %self.format_data).split("_")[-1]),
                            'style_slice_no': int(style_name.strip(".%s" %self.format_data).split("_")[-1]),
                            'modality': chosen_mod,
                            'dataset': self.getDataset(image_name),
                            'style_dataset': self.getDataset(style_name)}
                if self.second_style_slice:
                    out_item['second_slice'] = os.path.join(self.image_dir, second_slice)
                    out_item['second_slice_label'] = os.path.join(self.label_dir, second_slice_label)
                if self.format_data == 'npz':
                    # Extract additional metadata
                    metadata = np.load(os.path.join(self.image_dir, image_name))
                    out_item['affine']= metadata['img_affine']
                out.append(out_item)
            else:
                out_item = {'image_file': os.path.join(self.image_dir, image_name),
                            'label_file': os.path.join(self.label_dir, label_name),
                            'modality': chosen_mod,
                            'slice_no': int(image_name.strip(".%s" %self.format_data).split("_")[-1]),
                            'style_slice_no': int(image_name.strip(".%s" %self.format_data).split("_")[-1]),
                            'dataset': self.getDataset(image_name)}
                if self.format_data == 'npz':
                    # Extract additional metadata
                    metadata = np.load(os.path.join(self.image_dir, image_name))
    ...
    def monaifyInput_non_corr(self, fixed_seq = None):
        ...
        if self.max_dataset_size is not None:
            if self.max_dataset_size < len(all_labels):
                length_all = self.max_dataset_size
        if self.second_style_slice:
            raise Warning("Second style slice shouldn't be used with non corresponding images and labels."
                          "Deactivating it")
            self.second_style_slice = False
        ...
        while length_all > 0:
            ...
            if self.style_label_dir is None:
                style_label_file = None
            else:
            ...
            out_item = {'image_file': os.path.join(self.image_dir, image_name),
                        ...
                        'style_label_file': style_label_file,
                        'slice_no': int(label_name.strip(".%s" %self.format_data).split("_")[-1]),
                        'style_slice_no': int(image_name.strip(".%s" %self.format_data).split("_")[-1]),
                        'modality': chosen_mod,
                        'dataset': dataset,
    ...
    def multipleVolumesLoader(self):
        ...
        if self.format_data == 'npz':
            ...
            if self.second_style_slice:
                second_slice_loader = mt.LoadImageD(
                    keys=('second_slice'),
                    reader = "numpyreader",
                    npz_keys = ('img',)
                )
                second_slice_label_loader = mt.LoadImageD(
                    keys=('second_slice_label'),
                    reader = "numpyreader",
                    npz_keys = ('label',)
                )
        else:
        ...
        if self.non_corresponding_slices:
            both_loaders =[image_loader, label_loader, style_loader, style_label_loader]
        else:
            both_loaders = [image_loader, label_loader]
        if self.second_style_slice:
            both_loaders += [second_slice_loader, second_slice_label_loader]

        return mt.Compose(both_loaders)
    ...
    def switchStyles(self, input_dict, flags = None):
        ...
        for b in range(input_dict['label'].shape[0]):
            if flags[b]:
                ...
                new_style_slice_no = int(new_style.split("_")[-1].strip(".%s" %self.format_data))
                ...
                if isinstance(loaded['label_file'], monai.data.meta_tensor.MetaTensor):
                    loaded['label_file'] = np.asarray(loaded['label_file'])
                if isinstance(loaded['image_file'], monai.data.meta_tensor.MetaTensor):
                    loaded['image_file'] = np.asarray(loaded['image_file'])

                label_style = torch.from_numpy(loaded['label_file'])
                if len(label_style.shape) > 3:
                    label_style = label_style[..., 0, :]
                image_style = torch.from_numpy(loaded['image_file'])
                if len(image_style.shape) == 2:
                    image_style = image_style.unsqueeze(-1)
                # Pad to fit the self.size
                label_style = self.padder(label_style, islabel=True)
                image_style = self.padder(image_style, islabel=False)

                # Put channels first and transform
                image_style = image_style.permute(2, 0, 1)[0:1, ...]  # Channels on top
