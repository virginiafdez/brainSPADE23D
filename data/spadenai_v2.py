...
class SpadeNai():
    ...
    def __init__(self, opt, mode, **kwargs):
        ...
        self.format_data = opt.format_data
        ...
        self.second_style_slice = opt.second_style_slice
    ...
    def getIteratorFun(self, dataset):
        ...
        for volumes in dataset:
            ...
            if self.second_style_slice:
                second_slice_label, second_slice_style, _ = self.slicer(label_vol, image_vol, lesion=False)
                second_slice_label = second_slice_label.permute(2, 0, 1)
                second_slice_style = second_slice_style.permute(2, 0, 1)
            ...
            if self.second_style_slice:
                second_slice_style = self.normalize(torch.from_numpy(second_slice_style))
            ...
            if self.skullstrip:
                ...
                if self.second_style_slice:
                    second_slice_style = uvir.SkullStrip(second_slice_style, second_slice_label, value = -1)
            ...
            output_dict = {'image': image.type('torch.FloatTensor'),
            ...
            if self.second_style_slice:
                output_dict['second_style'] = second_slice_style
    ...
    def multipleVolumesLoader(self, cross_style_vol):
        ...
        image_loader = mt.LoadImageD(
            keys=('image_file'),
            reader = "numpyreader",
            npz_keys = npz_keys_img
        )

        label_loader = mt.LoadImageD(
            keys=('label_file'),
            reader = "numpyreader",
            npz_keys = npz_keys_lab
        )
    ...
    def getDataset(self, path, datasets = None):
    ...
    def setWorkingVolume(self, image_paths, label_path,style = None,
        ...
        :param kwargs: if format data is npy (not npz), nifti_path must be specified, with a path to
        a nifti file from which the affine and header and be borrowed and correspond to the space of the
        file you want to read from.
        :return:
        '''

        # Find dataset associated to label and image paths.
        label_dataset = self.getDataset(label_path.split("/")[-1], self.datasets)
        image_datasets = {}
        if style is None:
            for seq, path in image_paths.items():
                image_datasets[seq] = self.getDataset(path.split("/")[-1], self.datasets)
        ...
        if self.format_data == 'npy' and 'nifti_path' in kwargs:
            self.working_volume = OfflineVolume(image_paths, label_path, label_dataset, cut = self.cut,
                                                ...
                                                nifti_path=kwargs['nifti_path'])
        elif self.format_data == 'npy' and 'nifti_path' not in kwargs:
            ValueError("Format specified is npy, but nifti_path was not specified."
                       "Add nifti_path to the arguments provided to initialise the volume.")
        else:
            self.working_volume = OfflineVolume(image_paths, label_path, label_dataset, cut=self.cut,
                                                format_data=self.format_data,
                                                style_dataset=image_datasets, labels_style=style_label_paths)
    ...
    def iterateThroughWV(self, modality, number_slices = 1,  use_same_style_slice=True,
        ...
        for s in range(number_slices):
            ...
            if self.working_volume.slice_iterator[modality]['style_slice'] is not None \
                ...
                if self.working_volume.separate_styles_path is not None:
                    chosen_style = os.path.join(self.working_volume.separate_styles_path[modality],
                                                np.random.choice(os.listdir(self.working_volume.separate_styles_path[modality])))
                    chosen_style_mask = findSimilarName(chosen_style.split("/")[-1],
                                                        self.working_volume.separate_styles_path_mask,
                                                        slice_index=-1, extension=".npz")
                    style_slice = np.load(chosen_style)['img']
                    label_style_slice = np.load(chosen_style_mask)['label'][..., 0, :]
                    self.working_volume.slice_iterator[modality]['style_slice'] = style_slice
                    self.working_volume.slice_iterator[modality]['style_slice_label'] = label_style_slice
                    style_slice_no = int(chosen_style.split("/")[-1].split("_")[-1].strip(".npz"))
                    self.working_volume.slice_iterator[modality]['style_slice_no'] = style_slice_no
                else:
                ...
                # Normalise
                if len(style_slice.shape)==3:
                    style_slice = style_slice.permute(2, 0, 1)  # Channels on top
                else:
                    style_slice = style_slice.unsqueeze(0)
                label_style_slice = label_style_slice.permute(2, 0, 1)  # Channels on top
                style_slice = self.normalize(style_slice)
                if self.skullstrip:
                    style_slice = uvir.SkullStrip(style_slice, label_style_slice, value = -1)
    ...
    def setWorkingVolume_noIm(self, label_path, style_path, style_path_label,
                              style, fix_style, format_data = 'nii.gz'):

        '''
        Associates an Offline Volume object to the container.
        :param label_path: path to the Parcellation nifti file you want to generated
        :param style_path: path to the 2D NPZ styles that will be used to generated the images
        :param style_path_label: path to the 2D NPZ masks for the styles
        :param style: style of the images that will be used
        :param fix_style: whether you want to fix the style slice or not
        :param format_data: string, nii.gz, or npz
        :return:
        '''

        # Find dataset associated to label and image paths.
        self.working_volume = OfflineVolumeNoIm(label_path,
                                                separate_styles_path=style_path,
                                                separate_styles_path_mask=style_path_label,
                                                orig_dataset=style,
                                                cut = self.cut,
                                                style_dataset=style,
                                                fix_style=fix_style,
        ...
        self.working_volume.volumes['Parcellation'] = self.padder(torch.from_numpy(self.working_volume.volumes['Parcellation']),
                                                                  islabel = True) # Move channels first as if they were a batch

    def getSlicefromWV_noIm(self, slices, modalities):

        '''
        Returns a SPADE-compatible input dictionary in the form of a batch.
        The label is retrieved from self.working_volumes. The styles are random chosen from the style path.
        :param slice_: list of slices to retrieve from the label (length N)
        :param modality: list of string with modalities to retrieve from label (length N)
        :return: dictionary, compatible with brainSPADE net input.
        '''
        ...
        # Slice or slices.
        for s_ind, slice_ in enumerate(slices):
            if modalities[s_ind] not in self.working_volume.separate_styles_path.keys():
                raise ValueError("Unrecognised modality %s: must be one of %s" %(modalities[s_ind],
                                                                                 ",".join(self.working_volume.separate_styles_path.keys())))
            ...
            if self.working_volume.style_images[modalities[s_ind]]['image'] is None or not self.fix_seq:
                style_image_path = os.path.join(self.working_volume.separate_styles_path[modalities[s_ind]],
                                                np.random.choice(os.listdir(
                                                    self.working_volume.separate_styles_path[modalities[s_ind]])
                                                ))
                style_label_path = os.path.join(self.working_volume.separate_styles_path_mask,
                                                findSimilarName(style_image_path.split("/")[-1],
                                                self.working_volume.separate_styles_path_mask,
                                                extension=".npz"))
                style_image = np.load(style_image_path)['img']
                if len(style_image.shape) == 3:
                    style_image = self.padder(torch.from_numpy(style_image), islabel=False)[..., 1:2]
                else:
                    style_image = self.padder(torch.from_numpy(style_image).unsqueeze(-1), islabel=False)
                style_slice_no = int(style_image_path.split("/")[-1].split("_")[-1].strip(".npz"))

                if self.working_volume.style_images[modalities[s_ind]]['label'] is None:
                    style_label = np.load(style_label_path)['label']
                    if len(style_label.shape) == 4:
                        style_label = style_label[..., 0, :]
                    style_label = self.padder(torch.from_numpy(style_label), islabel=True)
                else:
                    style_label = self.working_volume.style_images[modalities[s_ind]]['label']

                if self.working_volume.slice_iterator[modalities[s_ind]]['style_slice_no'] is None:
                    self.working_volume.slice_iterator[modalities[s_ind]]['style_slice_no'] = style_slice_no
                if self.working_volume.style_images[modalities[s_ind]]['style_image_path'] is None:
                    self.working_volume.style_images[modalities[s_ind]]['style_image_path'] = style_image_path
                if self.working_volume.style_images[modalities[s_ind]]['style_label_path'] is None:
                    self.working_volume.style_images[modalities[s_ind]]['style_label_path'] = style_label_path

            else:
                style_image = self.working_volume.style_images[modalities[s_ind]]['image']
                style_label = self.working_volume.style_images[modalities[s_ind]]['label']

            # Post-process
            style_image = style_image.permute(2, 0, 1)  # Channels on top
            style_label = style_label.permute(2, 0, 1)  # Channels on top
            label_slice = label_slice.permute(2, 0, 1) # Channels on top
            ...
            # Skullstrip
            if self.skullstrip:
                style_image = uvir.SkullStrip(style_image, style_label, value = -1)

            # Set also image as label
            image_slice = deepcopy(style_image)
            ...
            if s_ind == 0:
                output_dict['image'] = image_slice.type('torch.FloatTensor').unsqueeze(0)
                output_dict['label'] = label_slice.type('torch.FloatTensor').unsqueeze(0)
                output_dict['style_image'] = style_image.type('torch.FloatTensor').unsqueeze(0)
                output_dict['slice_style_no'] = [self.working_volume.slice_iterator[modalities[s_ind]]['style_slice_no']]
                output_dict['this_seq'] = [modalities[s_ind]]
                output_dict['this_dataset'] = [self.getDataset(self.working_volume.style_images[modalities[s_ind]]['style_image_path'],
                                                               self.working_volume.style_datasets)]
                output_dict['slice_no'] = [slice_]
                output_dict['image_path'] =  [self.working_volume.style_images[modalities[s_ind]]['style_image_path']]
                output_dict['label_path'] = [self.working_volume.paths['Parcellation']]
                output_dict['image_slice_no'] = [slice_]
            else:
                output_dict['image'] = torch.cat([output_dict['image'],
                                                  image_slice.type('torch.FloatTensor').unsqueeze(0)],
                                                  dim = 0)
                output_dict['label'] = torch.cat([output_dict['label'],
                                                  ...
                                                  dim = 0)
                output_dict['style_image'] = torch.cat([output_dict['style_image'],
                                                        style_image.type('torch.FloatTensor').unsqueeze(0)],
                                                        dim = 0)
                output_dict['slice_style_no'].append(self.working_volume.slice_iterator[modalities[s_ind]]['style_slice_no'])
                output_dict['this_seq'].append(modalities[s_ind])
                output_dict['this_dataset'].append(self.getDataset(self.working_volume.style_images[
                                                                       modalities[s_ind]]['style_image_path'],
                                                               self.working_volume.style_datasets))
                output_dict['slice_no'].append(slice_)
                output_dict['image_path'].append(self.working_volume.style_images[modalities[s_ind]]['style_image_path'])
                output_dict['label_path'].append(self.working_volume.paths['Parcellation'])
                output_dict['image_slice_no'].append(slice_)

        return output_dict

    def iterateThroughWV_noIm(self, modality, batch_size, ):
        '''
        Iterates through the volume (modality)
        :param modality:
        :return:
        ...
        slices_ = list(range(self.working_volume.slice_iterator[modality]['current'],
                             min(self.working_volume.slice_iterator[modality]['current']+batch_size,
                                 self.working_volume.slice_iterator[modality]['max'])))
        output_dict =  self.getSlicefromWV_noIm(slices_, [modality]*batch_size, )

        self.working_volume.slice_iterator[modality]['current'] += len(slices_)

        return output_dict, self.working_volume.slice_iterator[modality]['current'] == \
               self.working_volume.slice_iterator[modality]['max']
...
class OfflineVolume():
    ...
    def __init__(self, image_paths, label_path, orig_dataset, cut, format_data, style_dataset = None,
                 labels_style = None, bound_normalization = False, separate_styles_path = None,
                 separate_styles_path_mask = None, **kwargs):
        ...
        :param format_data: string, either npy or npz. Specifies the format of the volumes that are loaded.
        If npy, you need to specify nifti_path through kwargs, pointing to a nifti file from which we can take
        the affine and the header
        ...
        :param separate_styles_path: dictionary containing paths to modality-based style datasets (2D)
        :param separate_styles_path_mask: root path to folder containing style images mask
        :param kwargs: nifti_path, nifti file that agrees with the dimensions of the npy file that we are reading,
        if format is npy.
        '''

        self.format_data = format_data.lower().replace(".", "")
        ...
        self.separate_styles_path = separate_styles_path
        self.separate_styles_path_mask = separate_styles_path_mask
        if self.format_data == 'npz':
            ...
            if 'nifti_path' not in kwargs.keys():
                ValueError("Format specified is npy, but nifti_path was not specified."
                           "Add nifti_path to the arguments provided to initialise the volume.")
            else:
                nifti_file = nib.load(kwargs['nifti_path'])
                self.affines['Parcellation'] = nifti_file.affine
                self.headers['Parcellation'] = nifti_file.header
        ...
        for seq, path in image_paths.items():
            ...
            if self.format_data == 'npz':
                ...
                self.affines['gt' + seq] = nifti_file.affine
                self.headers['gt' + seq] = nifti_file.affine
...
class OfflineVolumeNoIm():

    '''
        This class manages something called working_volume for cases where we only have the labels (NII),
        but we don't have corresponding images, but 2D style images.
        Meant to be used in offline (not using Persitent + Iterable dataset and Dataloaders) during test
        time.
        '''

    def __init__(self, label_path, separate_styles_path, separate_styles_path_mask,
                 orig_dataset, cut, style_dataset=None,
                 format_data='nii.gz',
                 bound_normalization=False, fix_style = True,  **kwargs):
        ...
        :param separate_styles_path_mask: root path to folder containing style images mask
        :param orig_dataset: Original dataset, to which the label belongs.
        :param format_data: str, format of the label data, '.nii.gz', '.npz'
        :param cut: string, 'a' (axial), 's' (sagittal) or 'c' (coronal) to be applied on the slicing process.
        :param fix_style: if True, once a style is fixed, it's left like that (modality-wise). IE: if you random
        pick a T1 style, that T1 will be used to generated all the slices.
        :param kwargs: nifti_path, nifti file that agrees with the dimensions of the npy file that we are reading,
        if format is npy.
        '''

        self.format_data = format_data
        if self.format_data == 'nii.gz':
            lab, lab_aff, lab_hea = self.loadVolume(label_path, label=True)
            self.volumes = {'Parcellation': lab}
        else:
            self.volumes = {'Parcellation': self.loadVolume(label_path, label=True)}
        ...
        self.non_corr = {}
        self.slice_iterator = {}
        self.style_datasets = orig_dataset
        self.cut = cut
        self.bound_normalization = bound_normalization
        self.cut_ids = self.cut_ids = {'a': 2, 's': 0, 'c': 1}
        self.separate_styles_path = separate_styles_path
        ...
        if self.format_data == 'npz':
            ...
            self.affines['Parcellation'] = metadata['label_affine']
            self.headers['Parcellation'] = metadata['label_header']
        elif self.format_data == 'nii.gz':
            self.affines['Parcellation'] = lab_aff
            self.headers['Parcellation'] = lab_hea
        else:
            if 'nifti_path' not in kwargs.keys():
                ValueError("Format specified is npy, but nifti_path was not specified."
                           "Add nifti_path to the arguments provided to initialise the volume.")
        ...
        self.style_images = {}
        for mod in separate_styles_path.keys():
            self.style_images[mod] = {'image': None, 'label': None,
                                      'style_label_path': None,
                                      'style_image_path': None}
            self.slice_iterator[mod] = None
            self.volumes['recon%s' %mod] = None
    ...
    def loadVolume(self, path, label=False):
        ...
        if self.format_data == 'npz':
            ...
            nifti = nib.load(path)
            return np.asarray(nifti.dataobj), nifti.affine, nifti.header
        else:
            npz_keys_label = None
            npz_keys_image = None
    ...
    def storeSlice(self, slice, modality):
        ...
        if self.volumes['recon' + modality] is None:
            self.volumes['recon' + modality] = slice.detach().cpu()
        else:
            self.volumes['recon' + modality] = torch.cat([self.volumes['recon' + modality],
                                                          slice.detach().cpu()], dim=0)
    ...
    def storeReconstruction(self, path, modality):
        ...
        nii_file = nib.Nifti1Image(out_vol.numpy(), self.affines['Parcellation'],
                                   )
        nib.save(nii_file, path)
