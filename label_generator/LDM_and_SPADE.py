...
For earlier LDM models_spade that do not produce PV maps, but Semantic Labels.
...
from label_generator.conditioned_ldm.training_and_testing.models_.ddim import DDIMSampler
...
# For stage 1
vae_model_path = "" # Path to the model .pth file
vae_config_file = "" #  YAML file for the VAE
ldm_model_path = "" # Path to the model .pth file
ldm_config_file = "" # YAML file for the LDM
...
ldm = loadmodel(ldm_model_path, ldm_config_file)
...
if stage_1 and not rendered_labels:
    ...
    while processed < n_samples and not rendered_labels:
        ...
        if n_samples - processed - batch_size < 0:
            ...
            b_size = batch_size
        sample_shape = tuple([b_size, vae.decoder.z_channels] + [j / (vae.encoder.num_resolutions - 1) \
                                                                 for j in vae.encoder.resolution[0]])  # Sample shape
        if use_conditioning:
