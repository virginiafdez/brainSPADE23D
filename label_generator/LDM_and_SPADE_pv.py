...
# For stage 1
vae_model_path = "/media/vf19/BigCrumb/JOURNAL_22/GENERATIVE_MODELS/MULTI_DISEASED_VAE/best_model.pth" # Path to the model .pth file
vae_config_file = "/media/vf19/BigCrumb/JOURNAL_22/GENERATIVE_MODELS/MULTI_DISEASED_VAE/multi-disease-vae.yaml" #  YAML file for the VAE
ldm_model_path = "/media/vf19/BigCrumb/JOURNAL_22/GENERATIVE_MODELS/MULTI_DISEASED_LDM_2H_FLOAT_GUID15/final_model.pth" # Path to the model .pth file
ldm_config_file = "/media/vf19/BigCrumb/JOURNAL_22/GENERATIVE_MODELS/MULTI_DISEASED_LDM_2H_FLOAT_GUID15/train_ldm_conditioned.yaml" # YAML file for the LDM

result_save_dir = "/media/vf19/BigCrumb/JOURNAL_22/GENERATED_DATASETS/DEBUG/train"
n_samples = 2000
batch_size = 8
guidance_scale= 3
...
override_label_dir = "/media/vf19/BigCrumb/JOURNAL_22/GENERATED_DATASETS/SABRE_HEALTHY_GUID15/test"
override_label_folder = "labels"
...
# Read labels and make sure they can be read, otherwise delete (corrupt)
if stage_purge:
    print("Purging...")
    label_dir = os.path.join(result_save_dir, 'labels')
    for l in tqdm(os.listdir(label_dir)):
        try:
            lab = np.load(os.path.join(label_dir, l))['label']
        except:
            print("Deleting... %s" %l)
            os.remove(os.path.join(label_dir, l))
            for m in modalities:
                if os.path.isdir(os.path.join(result_save_dir, 'images_%s' %m)):
                    result = findSimilarName(l,
                                             os.path.join(result_save_dir, 'images_%s' %m),
                                             slice_index=-2,
                                             extension=".npy",
                                             return_all=True,
                                             slice_ori_index=-1)
                    for r in result:
                        os.remove(os.path.join(result_save_dir, 'images_%s' %m, r))
...
if stage_2:
    ...
    for mod in modalities:
        ...
        if not stage_1 and override_label_dir is not None:
            opt.label_dir = os.path.join(override_label_dir, override_label_folder)
        else:
            opt.label_dir = os.path.join(result_save_dir, override_label_folder)
        opt.fix_seq = mod
